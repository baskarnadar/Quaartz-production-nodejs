...
exports.getCityAndStore = async (req, res, next) => {
 ...
 exports.getCityAndStoreSorting = async (req, res, next) => {
  ...
  // Get the current location's latitude, longitude, and searchtext from the request
  ...
  try {
      ...
      let query = [
          ...
          {
              // Project the fields you want, including the city info (EnCityName, ArCityName)
              $project: {
                  StoreID: 1,
                    // Add the Arabic Store name field
                  StoreLatitude: 1,
                  StoreLongitude: 1,
                  EnCityName:1,
      ...
      updatedResult.sort((a, b) => {
          ...
          // Default: Ascending order by distance
      });
...
exports.createStore = async (req, res, next) => {
  try {
    ...
    const result = await db.collection('tblStore').insertOne(storeitem);
